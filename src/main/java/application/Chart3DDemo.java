...
public class Chart3DDemo extends Application {
	...
	public void start(Stage primaryStage) {
		...
		float amplification = 50; // amplification of noise amplituda funkcji
									// (wysokość)

		// for (int x = 0; x < size; x++) {
		// for (int z = 0; z < size; z++) {
		// mesh.getPoints().addAll(x, noiseArray[x][z] * amplification, z);//tu
		// tworzy siatkę z 3 pkt
		// }
		// }
		F_mich funkcja = new F_mich();
		for (int x = 0; x < size; x++) {
			for (int z = 0; z < size; z++) {
				// Float f = (Float) funkcja.wykonaj((double)
				// noiseArray[x][z]).floatValue();
				mesh.getPoints().addAll(x, noiseArray[x][z] * amplification, z);// tu
				// mesh.getPoints().addAll(x, noiseArray[x][z], z);// tu
				// mesh.getPoints().addAll(x, funkcja.wykonaj(x,
				// z).floatValue(), z);// tu
				// tworzy
				// siatkę
				// z
		...
		// scene
		Scene scene = new Scene(root, 1600, 900, true, SceneAntialiasing.DISABLED);
	...
	/**
	 * Create an array of the given size with values of perlin noise Dla x i y
	 * tworzy wartość z
	 * 
	 * @param size
	 * @return
...
//				 double noise = 10 * Math.sin(x / 10) * Math.cos(y / 20) * x;
				// double noise = ImprovedNoise2.noise(x , y , 0);

				double noise = ImprovedNoise2.noise(x * frequency, y * frequency, 0);
				// noise = funkcja.wykonaj((double) noise);
	...
	public final static class ImprovedNoise2 {
		private static final double PI = 3.14159265;
		private static int n = 2;
		private static int m = 10;
		private static Double s = new Double(0);

		public static Double wykonaj(Double wartosc) {
		...
		public static Double wykonaj(int x1, int x2) {
			return 10 * Math.sin(x1 / 10) * Math.cos(x2 / 20) * x1;
//			Double s1 = Math.pow(3, (2.0 / PI));
