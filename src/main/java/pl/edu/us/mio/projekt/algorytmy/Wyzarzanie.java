...
//		int losP = (int) (random.nextInt(przedzial) + przedzial_od);
		int przedzial = (int) (przedzial_do);
		int losP = (int) random.nextInt(przedzial) ;
		punkt.setX(losP);
...
//				if (obr < 1000 || obr % dzielnik == 0)
					data.add(new XYChart.Data(obr, new Double(funkcja.wykonaj(punktLeader.x, punktLeader.y))));
			} catch (Exception e) {
...
//				}
	...
	private Punkt sasiad(Punkt punkt, int obr) {
		Double i = random.nextDouble() / 10;
		Double j = random.nextDouble() / 10;
		int przedzial = (int) (przedzial_do - przedzial_od);
		
		if (przedzial > 15 && epoka - obr > 10000){
...
//		}
	...
	public void run() {
		if (funkcja instanceof FunkcjaKwadratowa
		    || funkcja instanceof Funkcja2
		    || funkcja instanceof Funkcja3)
			wykonaj(1);
		else
			wykonaj();
	}
